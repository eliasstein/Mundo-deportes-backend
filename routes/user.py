...
def read_root(user_login:usermodel.login):
    ...
    resp.set_cookie(key="usrnm",value=usrdata["data"]["username"], max_age=3600)
    resp.set_cookie(key="localId",value=user["localId"], max_age=3600)
    resp.set_cookie(key="idToken",value=user["idToken"], max_age=3600)
    if user_login.remember:
        resp.set_cookie(key="refreshToken",value=user["refreshToken"])
    return resp
...
def refresh_token(request:Request):
    ...
    res.set_cookie(key="usrnm",value=usrdata["data"]["username"], max_age=3600)

    res.set_cookie(key="localId",value=user["userId"], max_age=3600)
    res.set_cookie(key="idToken",value=user["idToken"], max_age=3600)
    res.set_cookie(key="refreshToken",value=user["refreshToken"])
